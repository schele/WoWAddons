# TrinketBar (World of Warcraft AddOn)
...
character pane to swap it in. TrinketBar puts every trinket you own — worn or
...
up to you; TrinketBar does not decide it for you and does not care what is
...
**TrinketBar only equips.** It does not fire your trinket's on-use effect, and
...
TrinketBar's buttons say "equip this trinket", nothing more. A separate click
...
is a different, much bigger addon. TrinketBar does one job.
...
- **A trinket looted mid-fight does not get a button until the fight ends.**
  Every button TrinketBar will ever need is built at login, before you can be
...
1. Copy this folder into your client's `Interface/AddOns` as `TrinketBar`, so
   the result is `.../Interface/AddOns/TrinketBar/TrinketBar.toc`.
2. Restart the client and enable TrinketBar from the AddOns list.
...
`/trinketbar` or the short `/tb`:

- `/tb` - Show the command list
- `/tb lock` - Stop the bar being dragged
- `/tb reset` - Put the bar back in the middle
- `/tb settings` - Open the settings panel

## Settings

`/tb settings` opens a panel with a gem beside the title and three settings,
...
What the tests prove is that TrinketBar asks for the right thing: that the
